					...
					if (that.type == 'update') {
						...
						new Promise((resolve, reject) => {
							...
							updateImgDetail(that.imgInfo).then(res => {
								...
								uni.uploadFile({
									url: appConfig.WEB_API + '/utils/fileoss/uploadOssFiles',
									files: that.imgurl,
									formData: {
									...
									success: (res) => {
										...
										if (res.data.code === 0) {
											...
											newImg.cover = arr[0]
											newImg.imgsUrl = JSON.stringify(arr)
											newImg.status = 1
											newImg.type = 1
											updateStatus(newImg).then()
								...
								uni.uploadFile({
									...
									success: (res) => {
										...
										if (res.data.code === 0) {
											...
											newImg.cover = res.data.data[0]
											newImg.imgsUrl = JSON.stringify(res.data.data)
											newImg.status = 1
											newImg.type = 0
											updateStatus(newImg).then()
